...
export async function POST(request: Request) {
  try {
    ...
    // Fetch truck if truckId supplied
    ...
    // Fetch packages if shipmentId supplied and packages not provided
    ...
    // Run 3D AI Bin Packing Optimization
    const placedPackages = autoOptimize(targetPackages, targetTruck, validStrategy);
    const metrics = calculateSpaceMetrics(placedPackages, targetTruck);
    const weightDistribution = calculateWeightDistribution(placedPackages, targetTruck);
    const steps = generateLoadingSteps(placedPackages, targetTruck);
    const constraints = validateConstraints(placedPackages, targetTruck);
    const report = generateLoadingReport(placedPackages, targetPackages, targetTruck, validStrategy);

    return NextResponse.json({
      success: true,
      strategy: validStrategy,
      truck: targetTruck,
      totalRequestedPackages: targetPackages.length,
      placedPackageCount: placedPackages.length,
      placedPackages,
      steps,
      metrics,
      weightDistribution,
      stabilityAnalysis: report.stabilityAnalysis,
      damageRiskReport: report.damageRiskReport,
      constraints,
      unloadingReport: report.unloadingReport,
      report,
    });
  } catch (error: any) {
    console.error('Error running AI Load Optimization:', error);
    return NextResponse.json(
      { error: error.message || 'Failed to execute AI load optimization engine' },
      { status: 500 }
    );
  }
}
