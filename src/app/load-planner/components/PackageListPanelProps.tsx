...
function PriorityBadge({ priority }: { priority: string }) {
  ...
  return (
    <span className={`status-badge text-[9px] ${map[priority] || 'bg-muted text-muted-foreground'}`}>
      {priority}
    </span>
  );
}
...
export default function PackageListPanel({
  unplacedPackages, placedPackages, selectedPackageId,
  onSelectPackage, onPlacePackage, onRemovePackage, onHighlightPackage,
      ...
      <div className="flex border-b border-border">
        <button
          ...
          className={`flex-1 py-2.5 text-xs font-600 transition-colors ${
            activeTab === 'unplaced' ?'text-primary border-b-2 border-primary' :'text-muted-foreground hover:text-foreground'
          }`}
        >
        ...
        <button
          ...
          className={`flex-1 py-2.5 text-xs font-600 transition-colors ${
            activeTab === 'placed' ?'text-primary border-b-2 border-primary' :'text-muted-foreground hover:text-foreground'
          }`}
        >
          ...
          <>
            {unplacedPackages.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-8 text-center">
                ...
                <p className="text-[10px] text-muted-foreground mt-1">Review the 3D view and confirm loading.</p>
              </div>
            ) : (
              unplacedPackages.map((pkg) => (
                <div
                  ...
                  className={`card-elevated rounded-lg p-2.5 border transition-all cursor-pointer ${
                    selectedPackageId === pkg.id ? 'border-primary bg-primary/5' : 'border-border hover:border-border/80'
                  }`}
                ...
                >
                  <div className="flex items-start justify-between gap-1 mb-1.5">
                    ...
                    <div className="flex items-center gap-1 flex-shrink-0">
                      <span className="text-[9px] text-muted-foreground">#{pkg.deliverySequence}</span>
                    </div>
                  </div>
                  ...
                  {pkg.riskLevel === 'HIGH' && (
                    <div className="flex items-center gap-1 mb-1.5 p-1.5 bg-negative/10 rounded border border-negative/20">
                      ...
                      <span className="text-[9px] text-negative">High damage risk ({pkg.riskScore}/100)</span>
                    </div>
                  )}

                  <button
                    onClick={(e) => { e.stopPropagation(); onPlacePackage(pkg); }}
            ...
            {placedPackages.length === 0 ? (
              ...
              placedPackages.map((p) => (
                <div
                  key={p.package.id}
                  className={`card-elevated rounded-lg p-2.5 border transition-all cursor-pointer ${
                    selectedPackageId === p.package.id ? 'border-primary bg-primary/5' : 'border-border hover:border-border/80'
                  }`}
                  onClick={() => onSelectPackage(selectedPackageId === p.package.id ? null : p.package.id)}
                  onMouseEnter={() => onHighlightPackage(p.package.id)}
                  onMouseLeave={() => onHighlightPackage(null)}
                >
                  <div className="flex items-start justify-between gap-1 mb-1">
                    <div className="min-w-0">
                      <p className="text-[11px] font-600 text-foreground truncate">{p.package.name}</p>
                      <p className="text-[9px] text-muted-foreground">
                        Pos: ({Math.round(p.position.x)}, {Math.round(p.position.y)}, {Math.round(p.position.z)})cm
                      </p>
                    </div>
                    <div className={`status-badge text-[9px] ${
                      p.damageRisk > 60 ? 'risk-high' : p.damageRisk > 30 ? 'risk-medium' : 'risk-low'
                    }`}>
                      Risk {p.damageRisk}
                    </div>
                  </div>
                  ...
                  <div className="flex items-center gap-1 mb-1.5 flex-wrap">
                    ...
                    {p.position.rotationY === 90 && (
                      <span className="text-[9px] text-info bg-info/10 px-1 rounded">Rotated 90°</span>
                    )}
                  </div>

                  {p.damageReasons.filter(r => !r.includes('Low risk')).map((reason, i) => (
                    <div key={i} className="flex items-start gap-1 mb-1 p-1.5 bg-warning/10 rounded border border-warning/20">
                  ...
                  <button
                    onClick={(e) => { e.stopPropagation(); onRemovePackage(p.package.id); }}
